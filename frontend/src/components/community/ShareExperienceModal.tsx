...
import { CANONICAL_CATEGORIES } from '../../utils/communityTaxonomy';
...
export const ShareExperienceModal: React.FC<ShareExperienceModalProps> = ({
  ...
  const [category, setCategory] = useState<string>('Birds');
  ...
  const fetchAvailableActivities = async () => {
    try {
      ...
      // Optional background fetch, does not block posting
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!content.trim()) {
      setError('Please write about your nature observation or field experience.');
      return;
    }
    ...
    try {
      ...
      const res = await api.createExperiencePost({
        ...
        category: category || 'General Nature',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '18px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.78rem', color: '#64748b', margin: 0 }}>
              Post your sightings, wildlife observations, and photography to the BNHS community
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {attendedActivities.length > 0 && (
            <div>
              <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 700, color: '#1e293b', marginBottom: '6px' }}>
                Link to an Activity <span style={{ fontWeight: 400, color: '#64748b' }}>(Optional)</span>:
              </label>
              <select
              ...
              >
                <option value="">None / General Field Observation</option>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Share your trail notes, species sightings, photography tips, or memorable moments... Use #hashtags to help others discover it!"
              rows={4}
              style={{
                width: '100%',
                padding: '12px',
